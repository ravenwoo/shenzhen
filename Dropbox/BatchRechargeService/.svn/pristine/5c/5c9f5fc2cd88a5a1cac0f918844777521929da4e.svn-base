...
public class ProtocolUtil {
    ...
    private static BatchInputExecuteService batchInputExecuteService = new BatchInputExecuteServiceImpl();
    ...
    public static void rechargeRequest(BatchInputVo vo, int command) {
        ...
        try {
            ...
            if (result == 1 || result == 3) {//Result为1或3代表成功，其他为失败
                ...
                vo.setSendorderid("");
              //回写sendOrderId
                int update = batchInputExecuteService.updateSendOrderIdById(vo.getId(), "");
                if (update > 0) {
                    logger.info("充值接口返回结果为【失败】，将订单【" + vo.getId() + "】的sendorderid置空成功...");
                }
            }
